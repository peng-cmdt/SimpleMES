...
import { CheckCircle, XCircle, Clock, Settings, Play, Pause } from 'lucide-react';

interface USBScannerValidatorProps {
...
export default function USBScannerValidator({ 
  onValidationSuccess, 
  onValidationFailure, 
  className = '' 
}: USBScannerValidatorProps) {
  // 状态管理
  ...
  const getStatusIcon = () => {
    switch (validationStatus) {
      case 'scanning': return <Clock className="h-6 w-6 text-blue-500 animate-spin" />;
            ...
            <Alert className={
              ...
              <AlertDescription className="flex items-center gap-2">
                ...
                <span>
                  ...
                  {validationStatus === 'scanning' && (isPaused ? '扫描已暂停，点击恢复继续' : '请使用扫码枪扫描条码')}
                </span>
              </AlertDescription>
            </Alert>
          )}
